...
use rspack_core::{
  BuildMetaDefaultObject, BuildMetaExportsType, ConstDependency, ContextMode,
  ContextNameSpaceObject, ContextOptions, DependencyCategory, RuntimeGlobals, SpanExt,
};
...
use crate::{
  dependency::{
    ...
    amd_require_item_dependency::AMDRequireItemDependency, local_module::LocalModule,
    AMDRequireContextDependency,
  },
  utils::eval::BasicEvaluatedExpression,
  visitors::{context_reg_exp, create_context_dependency, JavascriptParser, Statement},
};

fn create_amd_require_context_dependency(
  ...
  return AMDRequireContextDependency::new(start, end, end, options, None, false);
}

impl<'parser> JavascriptParser<'parser> {
  // fn set_variable(&mut self, name: String) {
  //   self.definitions_db;
  // }
}
...
impl AMDDefineDependencyParserPlugin {
    ...
    if param.is_conditional() {
      ...
      let dep = if param_str == "require" {
        ...
        Box::new(ConstDependency::new(
          range.0,
          range.1,
          RuntimeGlobals::EXPORTS.name().into(),
        ...
        Box::new(ConstDependency::new(
          range.0,
          range.1,
          RuntimeGlobals::MODULE.name().into(),
  ...
  fn process_context(
    ...
    // TODO: current implementation is incorrect. fix it later
    create_amd_require_context_dependency(parser, param, call_expr.callee.as_expr().unwrap());
    Some(true)
  }
  ...
  fn process_call_define(
    ...
    let mut named_module: Option<&Expr> = None;
    ...
    match call_expr.args.len() {
      ...
      2 => {
        ...
        if first_arg.expr.is_lit() {
          // define("…", …)
          named_module = Some(&first_arg.expr);
    ...
    let dep = Box::new(AmdDefineDependency::new(
      ...
      named_module.and_then(|expr| get_lit_str(expr)),
    ));
...
impl JavascriptParserPlugin for AMDDefineDependencyParserPlugin {
  fn call(
    ...
    let is_define_call = if let Callee::Expr(callee_expr) = &call_expr.callee {
      callee_expr.is_ident_ref_to("define")
      // TODO: check if there is any other `define` in the scope?
    } else {
      false
    };
