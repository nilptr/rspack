...
use rspack_core::{
  AsyncDependenciesBlock, BoxDependency, ConstDependency, DependencyLocation,
  RealDependencyLocation, RuntimeGlobals, SpanExt,
};
...
impl AMDRequireDependenciesBlockParserPlugin {
  ...
  fn process_call_require(
    ...
    let block_loc = Some(DependencyLocation::Real(
      Into::<RealDependencyLocation>::into(call_expr.span).with_source(parser.source_map.clone()),
    ));
