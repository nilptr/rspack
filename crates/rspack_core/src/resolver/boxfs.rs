...
impl ResolverFileSystem for BoxFS {
  async fn read(&self, path: &std::path::Path) -> io::Result<Vec<u8>> {
    self.0.async_read(path.assert_utf8()).await.to_io_result()
  }

  async fn read_to_string(&self, path: &std::path::Path) -> std::io::Result<String> {
    match self.0.async_read(path.assert_utf8()).await {
      Ok(x) => String::from_utf8(x).map_err(|err| io::Error::new(io::ErrorKind::InvalidData, err)),
      Err(Error::Io(e)) => Err(e),
    }
  }

  async fn metadata(&self, path: &std::path::Path) -> std::io::Result<FileMetadata> {
    match self.0.async_metadata(path.assert_utf8()).await {
  ...
  async fn symlink_metadata(&self, path: &std::path::Path) -> std::io::Result<FileMetadata> {
    match self.0.async_symlink_metadata(path.assert_utf8()).await {
      Ok(meta) => Ok(FileMetadata {
        is_dir: meta.is_directory,
        is_file: meta.is_file,
        is_symlink: meta.is_symlink,
  ...
  async fn canonicalize(&self, path: &std::path::Path) -> std::io::Result<std::path::PathBuf> {
    match self.0.async_canonicalize(path.assert_utf8()).await {
